...
/**
 *  Modifications:
 *      - added collector bot - one that collects energy and returns to the master (spawned by master if it has enough energy)
 *      - collector bots use depth-first search to move back to the master bot
 *      - added landmine bot
...
object ControlFunction
{
    ...
    def forMaster(bot: Bot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeView(bot.view)
    ...
    def moveBot(bot: Bot, directionValue: Array[Double]) {
    ...
    def spawnCollector(bot: Bot) {
        val offset = XY(random.nextInt(3)-1, random.nextInt(3)-1)
        bot.spawn(offset, "mood" -> "Collecting")
        bot.say("Spawning collector bot")
    }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Collecting" => reactAsCollectorBot(bot)
            case "Being a landmine" => reactAsLandmineBot(bot)
            case s: String => bot.log("unknown mood: " + s)
        }
    }
    
    def reactAsLandmineBot(bot: MiniBot) {
        explodeIfNeeded(bot)
    }
    
    def reactAsCollectorBot(bot: MiniBot) {
        if (bot.energy > 500) {
            // return the energy to master bot    
            moveToMaster(bot)
        } else {
            ...
            if (target.isNonZero) {
                ...
                val offsetToGoodBeast = bot.view.offsetToNearest('B')
                
                offsetToGoodBeast match {
                    case Some(nearest: XY) =>
                        bot.move(nearest.signum)
                        bot.set("target" -> nearest)
                    case None =>
                        val offsetToGoodPlant = bot.view.offsetToNearest('P')
                        
                        offsetToGoodPlant match {
                            case Some(nearest: XY) =>
                                bot.move(nearest.signum)
                                bot.set("target" -> nearest)
                            case None =>
                                // randomly move around
                                val direction = XY(random.nextInt(3)-1, random.nextInt(3)-1)
    ...
    def moveToMaster(bot: MiniBot) {
        val masterOffset = bot.view.offsetToNearest('M') match {
            case None =>
                // move one step closer to master
                bot.move(bot.offsetToMaster.signum)
            case Some(nearest: XY) => 
                getCellToMoveTo(bot, nearest) match {
                    case None => 
                    case Some(cell) =>
                        val relativeCell = cell - bot.view.center
                        bot.move(relativeCell.signum)
                }
        }
    }
    
    def getCellToMoveTo(bot: MiniBot, masterOffset: XY): Option[XY] = {
        val matrix = buildViewMatrix(bot)
        
        // apply depth-first search
        val visitedCells: ListBuffer[XY] = ListBuffer()
        val masterCell = bot.view.center + masterOffset
        
        dfs(bot, masterOffset, matrix, visitedCells, masterCell)
    }
    
    def buildViewMatrix(bot: MiniBot): Array[Array[Char]] = {
        val matrix = Array.ofDim[Char](bot.view.size, bot.view.size)
        
        for (i <- 0 until bot.view.cells.length()) {
            matrix(i / bot.view.size)(i % bot.view.size) = bot.view.cells.charAt(i)
        }
        
        matrix
    }
    
    def dfs(bot: MiniBot, masterOffset: XY, matrix: Array[Array[Char]], visitedCells: ListBuffer[XY], currentCell: XY): Option[XY] = {
        visitedCells += currentCell
        
        val neighborCells = getNeighborCells(matrix, currentCell)
        var nextCell: Option[XY] = None
        
        neighborCells.foreach(cell => {
            if (cell == bot.view.center) {
                nextCell = Some(currentCell)
            } else {
                if (!visitedCells.contains(cell)) {
                    nextCell = dfs(bot, masterOffset, matrix, visitedCells, cell)
                }
            }
        })
    ...
    def getNeighborCells(matrix: Array[Array[Char]], cell: XY): ListBuffer[XY] = {
        val neighbors: ListBuffer[XY] = ListBuffer()
        
        val points: List[XY] = List(
            cell + XY.Up,
            cell + XY.RightUp,
            cell + XY.Right,
            cell + XY.DownRight,
        ...
        bot.view.offsetToNearest('m') match {
            ...
            case None =>
                ...
                if(target.isNonZero) {
                    ...
                    bot.set("mood" -> "Collecting", "target" -> "")
                    bot.say("Collecting")
                }
        }
    }
